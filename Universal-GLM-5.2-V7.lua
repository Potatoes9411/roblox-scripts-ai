...
local COLLECT_NAMES = { "coin", "token", "orb", "loot", "drop", "gem", "cash", "money", "star", "reward", "present", "gift" }
    ...
    track("collect", RunService.Heartbeat:Connect(function()
        ...
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") then
                -- method 1: proximity prompts (Rob/Bee Swarm hives etc.)
            ...
            elseif obj:IsA("BasePart") and not obj:IsDescendantOf(char) then
                local n = obj.Name:lower()
                local isCollectible = false
                for _, k in ipairs(COLLECT_NAMES) do if n:find(k) then isCollectible = true; break end end
                -- click detectors (Pet Sim / simulators)
                if obj:IsA("Model") or obj:IsA("BasePart") then
                    local cd = (obj:IsA("BasePart") and obj:FindFirstChildWhichIsA("ClickDetector"))
                        or obj:FindFirstChildWhichIsA("ClickDetector", true)
                    if cd and (cd.WorldPosition - root.Position).Magnitude < 60 then
                        pcall(function()
                            if fireclickdetector then fireclickdetector(cd) end
                        end)
                    end
                end
                -- method 2: magnet collectibles toward the player (touch-collect)
                if isCollectible and (obj.Position - root.Position).Magnitude < 120 then
                    pcall(function() obj.CFrame = root.CFrame + Vector3.new(0, 0, -3) end)
                end
            end
        end
    end))
...
-- SMART TELEPORT (name-based) ----------------------------------------
-- Finds a building/landmark by NAME anywhere in the workspace and
-- teleports to it. Works on ANY map layout (incl. your Studio copies),
-- unlike hardcoded coordinates.
local function findNamedLocation(keywords)
    local root = getRoot()
    local best, bd = nil, math.huge
    local function consider(pos, obj)
        ...
        elseif not best then best = obj end
    end
    for _, obj in ipairs(Workspace:GetDescendants()) do
        local n = obj.Name:lower()
        local matched = false
        for _, k in ipairs(keywords) do if n:find(k) then matched = true; break end end
        if matched then
            if obj:IsA("BasePart") then
                -- skip the local character's own parts
                if not (getChar() and obj:IsDescendantOf(getChar())) then
                    consider(obj.Position, obj)
                end
            elseif obj:IsA("Model") then
                local ok, p = pcall(function() return obj:GetPivot().Position end)
                if ok and p then consider(p, obj) end
            end
        end
    end
    return best
end
local function tpToNamedPart(keywords, label)
    local obj = findNamedLocation(keywords)
    if not obj then
        if label then Notify("Teleport", "Couldn't find '" .. label .. "' on this map.", Theme.Yellow) end
        return false
    end
    local pos
    if obj:IsA("BasePart") then pos = obj.Position
    else local ok, p = pcall(function() return obj:GetPivot().Position end); pos = ok and p or nil end
    if pos then
        tpTo(CFrame.new(pos) + Vector3.new(0, 6, 0))
        if label then Notify("Teleport", "→ " .. label) end
        return true
    end
    return false
end
...
-- Finds the REAL ball (workspace.Ball, or any fast-moving "ball"/"orb"
-- part), then clicks ONLY when time-to-impact is about to hit. Blade
-- Ball deflects by CLICKING / activating the sword — NOT a key press.
local function getBladeBall()
    -- 1) real Blade Ball keeps the ball as workspace.Ball
    local b = Workspace:FindFirstChild("Ball")
    if b and b:IsA("BasePart") then return b end
    -- 2) scan for a ball/orb part that is actually moving (the live one)
    local best, bestSpeed = nil, 30
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Parent then
            local n = obj.Name:lower()
            if n:find("ball") or n:find("orb") or n:find("projectile") then
                local sp = obj.AssemblyLinearVelocity.Magnitude
                if sp > bestSpeed then bestSpeed = sp; best = obj end
            end
        end
    end
    return best
end
local function bladeBallClick()
    local char = getChar()
    if char then
        for _, t in ipairs(char:GetChildren()) do
            if t:IsA("Tool") then pcall(function() t:Activate() end) end
...
local function parryPredictOn()
    ...
    track("parryP", RunService.Heartbeat:Connect(function()
        local root = getRoot(); if not root then return end
        local ball = getBladeBall(); if not ball then return end
        local toMe = root.Position - ball.Position
        local dist = toMe.Magnitude
        if dist < State.parryRange then
            local closing = ball.AssemblyLinearVelocity:Dot(toMe.Unit)
            if closing > 0 then
                local tti = dist / math.max(closing, 1)
                if tti < 0.4 then bladeBallClick() end
            elseif dist < 6 then
                bladeBallClick()
            end
        end
...
-- Attaches a BodyVelocity to the vehicle the player is DRIVING (found via
-- Humanoid.SeatPart -> vehicle model PrimaryPart / chassis), so it actually
-- flies instead of only searching the character's children.
local vFlyBV
local function getVehicleHost()
    local hum = getHum()
    -- 1) proper seated vehicle (Humanoid.SeatPart)
    local seat = hum and hum.SeatPart
    if not (seat and seat:IsA("VehicleSeat")) then
        -- 2) legacy: a vehicle seat nested inside the character
        local char = getChar()
        seat = char and char:FindFirstChildWhichIsA("VehicleSeat", true)
    end
    if not seat then return nil end
    -- find the owning vehicle model, then fly its chassis / primary part
    ...
    if model and model:IsA("Model") then
        ...
        for _, p in ipairs(model:GetDescendants()) do
            if p:IsA("BasePart") then
                local sz = p.Size.X * p.Size.Y * p.Size.Z
                if sz > bv then bv = sz; best = p end
            end
        end
...
local function nitroBoost()
    local host = getVehicleHost()
    if host then
        host.AssemblyLinearVelocity = host.CFrame.LookVector * 220
        Notify("Nitro", "Boost engaged!", Theme.Blue)
    else
        Notify("Nitro", "Enter a vehicle first.", Theme.Yellow)
    end
end
...
-- ANTI VOID (Tower of Hell / obbies) ---------------------------------
-- Remembers the last grounded CFrame and snaps the player back there the
-- instant they fall below the map or are about to die on a killbrick.
local lastSafe = nil
local function antiVoidOff() untrack("antivoid") end
local function antiVoidOn()
    untrack("antivoid")
    track("antivoid", RunService.Heartbeat:Connect(function()
        local hum = getHum(); local root = getRoot()
        if not hum or not root then return end
        local vel = root.AssemblyLinearVelocity
        local grounded = hum.FloorMaterial ~= Enum.Material.Air
        -- record a safe spot whenever standing on something solid
        if grounded and hum.Health > 0 and vel.Y > -5 then
            lastSafe = root.CFrame
        end
        -- fell below the map OR about to hit a killbrick at high speed
        local falling = root.Position.Y < -50 or (vel.Y < -120)
        if lastSafe and (falling or hum.Health <= 5) then
            root.CFrame = lastSafe + Vector3.new(0, 3, 0)
            root.AssemblyLinearVelocity = Vector3.zero
        end
    end))
end
local function resetToSafe()
    local root = getRoot()
    if lastSafe and root then
        root.CFrame = lastSafe + Vector3.new(0, 3, 0)
        Notify("Anti-Void", "Returned to last safe spot.", Theme.Green)
    else
        Notify("Anti-Void", "No safe spot recorded yet.", Theme.Yellow)
    end
end

--====================================================================
-- FLEE THE FACILITY: frozen-teammate rescue + PC hacking
-- Survivors win by hacking all PCs AND freeing frozen teammates. A frozen
-- player has a "Frozen"/"Captured" tag or is stuck (PlatformStanding).
-- Rescue = walk to them + fire rescue prompt / touch.
--====================================================================
local function isFrozenTeammate(plr)
    ...
    local stuck = root.AssemblyLinearVelocity.Magnitude < 1
    local tagged = char:FindFirstChild("Frozen") or char:FindFirstChild("Captured")
        or char:FindFirstChild("Trapped") or char:FindFirstChild("Grabbed")
    local isDown = (hum:GetState() == Enum.HumanoidStateType.PlatformStanding)
    return (tagged ~= nil) or (stuck and isDown)
end
local function autoRescueOff() untrack("ftfrescue") end
local function autoRescueOn()
    untrack("ftfrescue")
    trackThread("ftfrescue", 0.25, function()
        local root = getRoot(); if not root then return end
        local best, bd = nil, math.huge
        for _, p in ipairs(Players:GetPlayers()) do
            if isFrozenTeammate(p) and p.Character then
                local hrp = p.Character:FindFirstChild("HumanoidRootPart")
                if hrp then
                    local d = (hrp.Position - root.Position).Magnitude
                    if d < bd then bd = d; best = hrp end
        ...
        if best then
            if bd > 4 then
                root.CFrame = CFrame.new(best.Position + Vector3.new(0, 0, 3))
            else
                for _, o in ipairs(Workspace:GetDescendants()) do
                    if o:IsA("ProximityPrompt") and (o.WorldPosition - best.Position).Magnitude < 10 then
                        pcall(function() if fireproximityprompt then fireproximityprompt(o, 0) end end)
                    end
                end
                pcall(function() if firetouchinterest then firetouchinterest(root, best, 0) end end)
            end
        end
    end)
end
...
--====================================================================
-- DOORS: auto-hide in nearest closet/locker when a hostile entity spawns.
-- Rush/Ambush force you into a hiding spot; this ducks you into the nearest
-- closet/locker/wardrobe the moment one of those entities appears.
--====================================================================
local function doorsAutoHide()
    local root = getRoot(); if not root then return end
    local entity = nil
    for _, d in ipairs(Workspace:GetDescendants()) do
        if d:IsA("Model") then
            ...
            or n:find("figure") or n:find("halt") then
                entity = d; break
            end
        end
    end
    if not entity then Notify("Doors", "No hostile entity detected right now.", Theme.Yellow); return end
    local best, bd = nil, math.huge
    for _, d in ipairs(Workspace:GetDescendants()) do
        if d:IsA("BasePart") then
            local n = d.Name:lower()
            if n:find("closet") or n:find("locker") or n:find("wardrobe")
            or n:find("bed") or n:find("hide") then
                local dd = (d.Position - root.Position).Magnitude
                if dd < bd then bd = dd; best = d end
            end
        end
    end
    if best and bd < 200 then
        root.CFrame = best.CFrame + Vector3.new(0, 3, 0)
        for _, o in ipairs(best:GetDescendants()) do
            if o:IsA("ProximityPrompt") then
                pcall(function() if fireproximityprompt then fireproximityprompt(o, 0) end end)
            end
        end
        Notify("Doors", "Hidden! Entity: " .. entity.Name, Theme.Green)
    else
        Notify("Doors", "No hiding spot nearby!", Theme.Red)
    end
end
local function doorsHideLoopOff() untrack("doorshide") end
local function doorsHideLoopOn()
    untrack("doorshide")
    trackThread("doorshide", 0.3, function()
        local root = getRoot(); if not root then return end
        for _, d in ipairs(Workspace:GetDescendants()) do
            if d:IsA("Model") then
                local n = d.Name:lower()
                if n:find("rush") or n:find("ambush") or n:find("halt") then
                    local best, bd = nil, math.huge
                    for _, p in ipairs(Workspace:GetDescendants()) do
                        if p:IsA("BasePart") then
                            local pn = p.Name:lower()
                            if pn:find("closet") or pn:find("locker") or pn:find("wardrobe") then
                                local dd = (p.Position - root.Position).Magnitude
                                if dd < bd then bd = dd; best = p end
                            end
                        end
                    end
                    if best then root.CFrame = best.CFrame + Vector3.new(0, 2, 0) end
                    return
                end
            end
        end
...
--====================================================================
-- DA HOOD: calisthenics (gym) strength training + cash pickup.
-- Da Hood: gain stats at the gym (spam spacebar/E) and earn money by
-- picking up dropped cash bags (touch-collect).
--====================================================================
local function dhCalisthenicsOff() untrack("dhgym") end
local function dhCalisthenicsOn()
    untrack("dhgym")
    trackThread("dhgym", 0.25, function()
        local root = getRoot(); if not root then return end
        local atGym = false
        for _, p in ipairs(Workspace:GetDescendants()) do
            if p:IsA("BasePart") then
                local n = p.Name:lower()
                if (n:find("weight") or n:find("gym") or n:find("bench") or n:find("treadmill"))
                and (p.Position - root.Position).Magnitude < 12 then
                    atGym = true; break
                end
            end
        end
        if atGym then
            pressKey(Enum.KeyCode.Space); pressKey(Enum.KeyCode.E); clickMouse()
        end
    end)
end
local function dhPickupCash()
    local root = getRoot(); if not root then return 0 end
    local n = 0
    for _, o in ipairs(Workspace:GetDescendants()) do
        if o:IsA("BasePart") and not o:IsDescendantOf(getChar() or {}) then
            local nm = o.Name:lower()
            if (nm:find("cash") or nm:find("money") or nm:find("drop")) and (o.Position - root.Position).Magnitude < 90 then
                pcall(function() o.CFrame = root.CFrame + Vector3.new(0, 0, -3) end)
                n = n + 1
            end
        end
    end
...
local function dhCashLoopOff() untrack("dhcash") end
local function dhCashLoopOn()
    untrack("dhcash")
    trackThread("dhcash", 0.3, function() dhPickupCash() end)
end

--====================================================================
-- BEE SWARM: walk the field to collect pollen.
-- Pollen is gathered by holding your tool while moving across a field.
-- This walks a slow zig-zag across the nearest field while auto-firing.
--====================================================================
local function beeFieldFarmOff() untrack("beefield") end
local function beeFieldFarmOn()
    untrack("beefield")
    local t = 0
    trackThread("beefield", 0.1, function()
        local root = getRoot(); local char = getChar()
        if not root or not char then return end
        for _, tool in ipairs(char:GetChildren()) do
            if tool:IsA("Tool") then pcall(function() tool:Activate() end) end
        end
        local field, bd = nil, math.huge
        for _, p in ipairs(Workspace:GetDescendants()) do
            if p:IsA("BasePart") then
                local n = p.Name:lower()
                if n:find("field") and (p.Position - root.Position).Magnitude < 300 then
                    local d = (p.Position - root.Position).Magnitude
                    if d < bd then bd = d; field = p end
                end
            end
        end
        if field then
            t = t + 0.1
            local cx = math.sin(t) * 25
            local cz = math.cos(t * 0.6) * 25
            local goal = CFrame.new(field.Position.X + cx, field.Position.Y + 6, field.Position.Z + cz)
...
local function unlockDoors()
    ...
    for _, p in ipairs(Workspace:GetDescendants()) do
        if p:IsA("BasePart") then
            local n = p.Name:lower()
            if n:find("door") or n:find("gate") or n:find("barrier")
            or n:find("laser") or n:find("forcefield") or n:find("vault")
            or n:find("cell") or n:find("entry") or n:find("shutter") then
                pcall(function() p.CanCollide = false end)
                -- break welds/constraints that hold the door shut
                for _, c in ipairs(p:GetJoints()) do
                    if c:IsA("Weld") or c:IsA("WeldConstraint")
...
--====================================================================
_G.antiFlashOff,_G.antiFlashOn,_G.bunnyHopOff,_G.bunnyHopOn,_G.mm2RoleOff,_G.mm2RoleOn,_G.parryPredictOff,_G.parryPredictOn,_G.autoHatchOff,_G.autoHatchOn,_G.autoFarmNPCOff,_G.autoFarmNPCOn,_G.fastAttackOff,_G.fastAttackOn,_G.vehicleFlyOff,_G.vehicleFlyOn,_G.nitroBoost,_G.bringNearestItem,_G.bringItemsOff,_G.bringItemsOn,_G.noKillbrickOff,_G.noKillbrickOn,_G.unlockDoors,_G.parachuteOff,_G.parachuteOn,_G.bladeBallClick,_G.getBladeBall,_G.antiVoidOff,_G.antiVoidOn,_G.resetToSafe,_G.isFrozenTeammate,_G.autoRescueOff,_G.autoRescueOn,_G.doorsAutoHide,_G.doorsHideLoopOff,_G.doorsHideLoopOn,_G.dhCalisthenicsOff,_G.dhCalisthenicsOn,_G.dhCashLoopOff,_G.dhCashLoopOn,_G.beeFieldFarmOff,_G.beeFieldFarmOn = antiFlashOff,antiFlashOn,bunnyHopOff,bunnyHopOn,mm2RoleOff,mm2RoleOn,parryPredictOff,parryPredictOn,autoHatchOff,autoHatchOn,autoFarmNPCOff,autoFarmNPCOn,fastAttackOff,fastAttackOn,vehicleFlyOff,vehicleFlyOn,nitroBoost,bringNearestItem,bringItemsOff,bringItemsOn,noKillbrickOff,noKillbrickOn,unlockDoors,parachuteOff,parachuteOn,bladeBallClick,getBladeBall,antiVoidOff,antiVoidOn,resetToSafe,isFrozenTeammate,autoRescueOff,autoRescueOn,doorsAutoHide,doorsHideLoopOff,doorsHideLoopOn,dhCalisthenicsOff,dhCalisthenicsOn,dhCashLoopOff,dhCashLoopOn,beeFieldFarmOff,beeFieldFarmOn
end
...
local function isCopTool(n)
    n = (n or ""):lower()
    return n:find("handcuff") or n:find("arrest") or n:find("taser")
        or n:find("baton") or n:find("police") or n:find("cuff")
end
local function equipCopTool(char)
    -- equip a police/arrest tool from the backpack if one isn't already held
    local held = char:FindFirstChildWhichIsA("Tool")
    if held and isCopTool(held.Name) then return held end
    local bp = LocalPlayer:FindFirstChild("Backpack")
    local hum = getHum()
    if bp and hum then
        for _, t in ipairs(bp:GetChildren()) do
            if t:IsA("Tool") and isCopTool(t.Name) then
...
local function autoArrestOn()
    ...
    trackThread("arrest", 0.15, function()
        ...
        -- stand right behind the suspect, close + facing them (Jailbreak needs ~3-6 studs)
        root.CFrame = CFrame.new(thrp.Position - thrp.CFrame.LookVector * 4, thrp.Position)
        -- equip a cop tool first (handcuff / taser / baton) — required for arrest
        equipCopTool(char)
        -- method 1: most games arrest on a click aimed at the player
        clickMouse()
        -- method 2: activate the held police tool
        for _, t in ipairs(char:GetChildren()) do
            if t:IsA("Tool") and isCopTool(t.Name) then
...
local function addTeleportTab(tab, key)
    ...
    tab:AddLabel("Searches the map by name first, then uses preset coords.", Theme.Faint)
    for _, loc in ipairs(list_) do
        tab:AddButton("📍  " .. loc.name, function()
            -- try a name-based search first (works on ANY map layout)
            local words = {}
            for w in string.gmatch(loc.name:lower(), "%a+") do
                if #w >= 3 then table.insert(words, w) end
            end
            local found = (#words > 0) and tpToNamedPart(words)
            if not found then
                -- fall back to the preset coordinates
                if loc.pos[1] == "up" then
                    tpUp(loc.pos[2])
                else
                    tpCoords(loc.pos[1], loc.pos[2], loc.pos[3])
                end
                Notify("Teleport", "→ " .. loc.name .. " (preset coords)")
            end
        end)
    end
    tab:AddDivider()
...
local function buildRichFeatures(tab, g)
    ...
    if key == "jailbreak" then
        ...
        tab:AddSection("SMART TELEPORTS (find by name)")
        tab:AddLabel("Searches the map for these buildings — works on any layout.", Theme.Faint)
        tab:AddButton("🏦  Bank",        function() tpToNamedPart({ "bank" }, "Bank") end)
        tab:AddButton("💎  Jewelry",     function() tpToNamedPart({ "jewelry", "jewel" }, "Jewelry") end)
        tab:AddButton("🖼️  Museum",      function() tpToNamedPart({ "museum" }, "Museum") end)
        tab:AddButton("⛽  Gas Station", function() tpToNamedPart({ "gas", "fuel", "station" }, "Gas Station") end)
        tab:AddButton("🍩  Donut Shop",  function() tpToNamedPart({ "donut", "doughnut" }, "Donut Shop") end)
        tab:AddButton("⚡  Power Plant", function() tpToNamedPart({ "power", "plant" }, "Power Plant") end)
        tab:AddButton("🚔  Police HQ",   function() tpToNamedPart({ "police", "hq", "station" }, "Police HQ") end)
        tab:AddButton("⛓️  Prison",      function() tpToNamedPart({ "prison", "jail", "cell" }, "Prison") end)
        tab:AddButton("🚗  Car Spawn",   function() tpToNamedPart({ "car", "vehicle", "dealership", "garage" }, "Vehicle") end)
        tab:AddButton("🏪  Shop",        function() tpToNamedPart({ "shop", "store", "market" }, "Shop") end)
        return true
    end
    ...
    if key == "bladeball" then
        ...
        tab:AddKeybind("Manual Parry Key", Enum.KeyCode.F, function() bladeBallClick() end)
        tab:AddSection("VISUALS")
    ...
    if key == "toh" then
        ...
        tab:AddToggle("🛟  Anti-Void (auto-rescue)", false, function(v) if v then antiVoidOn() else antiVoidOff() end end, true)
        tab:AddButton("↩  Return to Safe Spot", resetToSafe)
    ...
    if key == "dahood" then
        ...
        tab:AddToggle("Kill Aura", false, function(v) State.killAura = v; if v then auraOn() else auraOff() end end)
        tab:AddSection("FARM / MONEY")
        tab:AddToggle("💵  Auto Pickup Cash", false, function(v)
            if v then dhCashLoopOn() else dhCashLoopOff() end end, true)
        ...
        tab:AddSection("GYM / STATS")
        tab:AddToggle("🏋️  Auto Calisthenics (gym)", false, function(v)
            if v then dhCalisthenicsOn() else dhCalisthenicsOff() end end)
        tab:AddLabel("Stand near gym weights, then enable.", Theme.Faint)
        tab:AddSection("MOVEMENT")
    ...
    if key == "beeswarm" then
        ...
        tab:AddToggle("🐝  Walk Field (auto pollen)", false, function(v)
            if v then beeFieldFarmOn() else beeFieldFarmOff() end end)
    ...
    if key == "fleetfacility" then
        ...
        tab:AddSection("RESCUE TEAMMATES")
        tab:AddToggle("🆘  Auto Rescue Frozen", false, function(v)
            if v then autoRescueOn() else autoRescueOff() end end, true)
        tab:AddButton("🔍  Check for Frozen Players", function()
            local n = 0
            for _, p in ipairs(Players:GetPlayers()) do if isFrozenTeammate(p) then n = n + 1 end end
            Notify("Flee", n .. " frozen teammate(s) detected.", n > 0 and Theme.Green or Theme.Yellow)
        end)
        tab:AddSection("UTILITY")
    ...
    if key == "doors" then
        ...
        tab:AddToggle("No Fall Damage", false, function(v) if v then noFallOn() else noFallOff() end end)
        tab:AddSection("SURVIVE ENTITIES")
        tab:AddToggle("🚪  Auto-Hide (Rush/Ambush)", false, function(v)
            if v then doorsHideLoopOn() else doorsHideLoopOff() end end, true)
        tab:AddButton("🏃  Hide Now (nearest closet)", doorsAutoHide)
        tab:AddSection("MOVEMENT")
        tab:AddToggle("Fly", false, function(v) State.fly = v; if v then flyOn() else flyOff() end end)
        tab:AddToggle("Noclip", false, function(v) State.noclip = v; if v then noclipOn() else noclipOff() end end)
        return true
...
local function buildGameFeatures(tab, key)
    ...
    or key == "obstacle" or key == "escapeschool" or key == "skyrace" then
        ...
        tab:AddToggle("Infinite Jump", false, function(v) State.infJump = v; if v then infJumpOn() else infJumpOff() end end)
        tab:AddToggle("Noclip", false, function(v) State.noclip = v; if v then noclipOn() else noclipOff() end end)
        tab:AddToggle("🛟  Anti-Void (auto-rescue)", false, function(v) if v then antiVoidOn() else antiVoidOff() end end)
